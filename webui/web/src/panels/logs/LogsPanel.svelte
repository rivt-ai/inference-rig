            ...
            <ul class="divide-y">
              {#each visibleEvents as event (event.id)}
                <li class={`flex items-center gap-3 px-3 py-2 text-sm ${event.success ? '' : 'bg-destructive/5'}`}>
                  ...
                  <span class="min-w-0 flex-1 truncate font-medium" title={event.detail || undefined}>{event.action}{event.profile ? ` ${event.profile}` : ''}{event.recovery ? ` ${event.recovery}` : ''}{event.detail ? `: ${event.detail}` : ''}</span>
