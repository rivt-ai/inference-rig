...
- `phase-05-backend-contracts` (this PR): Phase 5 neutral backend contract seam. Builds green, `go test ./...` passes, `golangci-lint run ./...` = 0 issues, neutralization grep clean.
- **Top of stack:** `phase-05-backend-contracts`. **Next:** Phase 6 (llama.cpp backend) on `phase-06-llamacpp`, based on `phase-05-backend-contracts`.
