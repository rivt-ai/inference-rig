...
- `surface-s5-setup`: llamarig-informed full-screen setup owns local first-run config creation/rerun backups and capability-aware backend installation. It writes no profile: profiles are created in the TUI or web UI, where the catalog is browsable and a model can be downloaded in the same step, and both surfaces warn while none exists.
...
- What exists now (added in Phase 10):
  ...
  - `core/setup.Wizard` — local config-existence first-run detection, terminal/safety checks, full-screen review/cancellation, safe direct config replacement, and optional backend installation through RPC. Global config never crosses RPC, and no profile is written.
